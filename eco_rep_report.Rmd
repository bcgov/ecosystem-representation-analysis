...
tar_load(c("map_pa_background", "map_eco_bec_background", "pa_eco_bec_summary", "pa_eco_bec_summary_wide", 
           "map_bec_background", "pa_bec_summary_wide", "map_eco_background"))
```
...
p <- ggplot(comp_by_rep_bec, aes(x = percent_comp_prov, y = percent_conserved_ppa, colour = zone)) +
  ...
  labs(title = "Percent of BGC variant conserved in PPAs vs Composition in B.C.",
       x = "Percent variant composotion in B.C.",
...
- What threshold do we set for representation?
...
Variants can be spread widely across the province, so we intersect with Ecoregions to 
try to achieve regional as well as ecological representation

```{r map-eco-bec-subset}
map_eco_bec <- map_eco_bec_background %>%
...
eco_var_rep_map_with_pa <- eco_rep_map +
  geom_sf(data = map_pa_background, mapping = aes(colour = pa_type), alpha = 0.4, size = 0.2) +
...
prov_eco_bec_summary_sf <- map_eco_bec_background %>% 
  left_join(pa_eco_bec_summary_wide, 
            by = c("ecoregion_name", "ecoregion_code", "zone", "subzone", "variant"))
...
p <- ggplot(comp_by_rep_eco_bec, 
  ...
  geom_point(aes(text = paste0(ecoregion_name, "*", bec_variant))) +
    scale_colour_manual(values = bec_cols, breaks = names(bec_cols)) + 
  labs(title = "Percent of BEC Variant x Ecoregion conserved in PPAs vs Composition",
       x = "Percent variant x ecoregion composotion in B.C.",
...
rare_variants <- filter(pa_bec_summary_wide, 
                        percent_comp_prov < quantile(percent_comp_prov, .1))

ggplot() + 
  geom_bc + 
  geom_sf(
    data = prov_eco_bec_summary_sf %>% 
      filter(percent_conserved_ppa < 15, 
...
eco_rep_barplot <- ggplot(dplyr::filter(pa_eco_bec_summary, ecoregion_code %in% ecoregion_subset)) +
  geom_col(aes(x = reorder_within(bec_variant, percent_conserved, ecoregion_name, fun = sum), y = percent_conserved, fill = pa_type)) +
